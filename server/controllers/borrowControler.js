...
export const recordBorrowedBook = catchAsyncErrors(async (req, res, next) => {
    ...
    res.status(200).json({
        success: true,
        message: "Borrowed book recorded succesfully.",
    });
});
...
export const returnBorrowBook = catchAsyncErrors(async (req, res, next) => { 
    ...
    if(!borrow){
        return next(new ErrorHandler("You have not borrowed this book"))
    }
    ...
    res.status(200).json({
        success: true,
        message: fine !== 0 ? `The book has been returned succesfully. Total charges including fine are$${fine + book.price}` : `The book has been returned succesfully. The total charges are $${book.price}`,
    })
});
